...
class FarmNotification {
  ...
  factory FarmNotification.fromSupabase(Map<String, dynamic> data) {
    return FarmNotification(
      ...
      dedupeKey: data['dedupe_key']?.toString(),
    );
  }
}
...
class FarmerPayout {
  ...
  // Existing retail payout link.
  // Wholesale payouts may have no retail order.
  final String orderId;
  ...
  final double? payoutQuantity;
  final String payoutUnit;
  ...
  const FarmerPayout({
    ...
    this.sourceType = 'retail',
    this.receivingBatchId = '',
  ...
  factory FarmerPayout.fromSupabase(
    ...
    return FarmerPayout(
      ...
      farmerId: (data['farmer_id'] ?? '').toString(),
      orderId: (data['order_id'] ?? '').toString(),
      grossAmount: Product._toDouble(
        data['gross_amount'],
      ),
      commissionAmount: Product._toDouble(
        data['commission_amount'],
      ),
      netAmount: Product._toDouble(
        data['net_amount'],
      ),
      payoutStatus:
          (data['payout_status'] ?? 'pending').toString().trim().toLowerCase(),
      payoutMethod: (data['payout_method'] ?? '').toString().trim(),
      payoutReference: (data['payout_reference'] ?? '').toString().trim(),
      sourceType:
          (data['source_type'] ?? 'retail').toString().trim().toLowerCase(),
      receivingBatchId: (data['receiving_batch_id'] ?? '').toString().trim(),
      wholesaleRequestId:
          (data['wholesale_request_id'] ?? '').toString().trim(),
      productName: (data['product_name'] ?? '').toString().trim(),
      payoutQuantity: data['payout_quantity'] == null
          ? null
          : Product._toDouble(
              data['payout_quantity'],
            ),
      payoutUnit: (data['payout_unit'] ?? '').toString().trim(),
      farmerUnitCost: data['farmer_unit_cost'] == null
          ? null
          : Product._toDouble(
  ...
  bool get isWholesaleReceiving => sourceType == 'wholesale_receiving';

  String get sourceLabel => isWholesaleReceiving ? 'Wholesale' : 'Retail';

  String get quantityLabel {
    final quantity = payoutQuantity;
    ...
    if (payoutUnit.trim().isEmpty) {
      return number;
    }

    return '$number $payoutUnit';
  }

  String get lotCode {
    if (!isWholesaleReceiving || notes.trim().isEmpty) {
      return '';
    }

    const marker = 'receiving lot ';
  ...
  String get shortSourceId {
    final sourceId = isWholesaleReceiving ? wholesaleRequestId : orderId;

    final clean = sourceId.replaceAll('-', '').trim().toUpperCase();
...
class SupportTicket {
  ...
  factory SupportTicket.fromSupabase(Map<String, dynamic> data) {
    return SupportTicket(
      ...
      lastMessagePreview: (data['last_message_preview'] ?? '').toString(),
      lastSenderRole: (data['last_sender_role'] ?? '').toString(),
      assignedStaffId: data['assigned_staff_id']?.toString(),
  ...
  String get preview {
    final clean = lastMessagePreview.trim();
  ...
  bool get hasUnreadForCustomer {
    ...
    final readAt = customerLastReadAt;
    return readAt == null || sentAt.isAfter(readAt);
  }

  bool get hasUnreadForStaff {
    if (lastSenderRole.trim().toLowerCase() != 'user') return false;
    final sentAt = lastMessageAt;
    if (sentAt == null) return false;
    final readAt = staffLastReadAt;
    return readAt == null || sentAt.isAfter(readAt);
  }
...
class SupportMessage {
  ...
  factory SupportMessage.fromSupabase(Map<String, dynamic> data) {
    return SupportMessage(
      ...
      senderRole: (data['sender_role'] ?? 'user').toString(),
      message: (data['message'] ?? '').toString(),
...
// =====================================================
// FARMER SUPPLY FORECAST
// =====================================================

class FarmerSupplyForecast {
  final String id;
  final String farmerId;
  final String cropName;
  final String? category;

  final double? quantityGrowing;
  final double? expectedQuantity;
  final double? harvestedQuantity;
  final double? hpjConfirmedQuantity;
  ...
  const FarmerSupplyForecast({
    ...
    this.category,
    this.quantityGrowing,
    this.expectedQuantity,
    this.harvestedQuantity,
    this.hpjConfirmedQuantity,
    required this.unit,
  ...
  factory FarmerSupplyForecast.fromSupabase(
    Map<String, dynamic> data,
  ) {
    return FarmerSupplyForecast(
      id: (data['id'] ?? '').toString(),
      farmerId: (data['farmer_id'] ?? '').toString(),
      cropName: (data['crop_name'] ?? '').toString().trim(),
      category: _nullableString(data['category']),
      quantityGrowing: _toNullableDouble(
        data['quantity_growing'],
      ),
      expectedQuantity: _toNullableDouble(
        data['expected_quantity'],
      ),
      harvestedQuantity: _toNullableDouble(
        data['harvested_quantity'],
      ),
      hpjConfirmedQuantity: _toNullableDouble(
        data['hpj_confirmed_quantity'],
      ),
      unit: (data['unit'] ?? 'lb').toString().trim(),
      expectedHarvestDate: _parseDate(
        data['expected_harvest_date'],
      ),
      harvestedAt: _parseDate(
        data['harvested_at'],
      ),
      status: (data['status'] ?? 'growing').toString().trim().toLowerCase(),
  ...
  bool get isExpected => status == 'expected';

  bool get isHarvestReady => status == 'harvest_ready';

  bool get isHarvested => status == 'harvested';

  bool get isHpjConfirmed => status == 'hpj_confirmed';
...
// =====================================================
// NAVIGATION MEMORY
// Remembers the last useful workspace and bottom tab for
// a signed-in HPJ account. Temporary detail pages are not
// persisted so the app always resumes somewhere safe.
// =====================================================
class HpjNavigationPreference {
  final String lastWorkspace;
  final int customerTab;
  final int farmerTab;
  final int wholesaleTab;

  const HpjNavigationPreference({
    this.lastWorkspace = 'customer',
    this.customerTab = 0,
    this.farmerTab = 0,
    this.wholesaleTab = 0,
  });

  factory HpjNavigationPreference.fromSupabase(
    Map<String, dynamic> data,
  ) {
    int safeTab(dynamic value) {
      final parsed = value is num
          ? value.toInt()
          : int.tryParse(value?.toString() ?? '') ?? 0;
      return parsed.clamp(0, 4).toInt();
    }

    final workspace =
        (data['last_workspace'] ?? 'customer').toString().trim().toLowerCase();

    return HpjNavigationPreference(
      lastWorkspace:
          const {'customer', 'farmer', 'wholesale'}.contains(workspace)
              ? workspace
              : 'customer',
      customerTab: safeTab(data['customer_tab']),
      farmerTab: safeTab(data['farmer_tab']),
      wholesaleTab: safeTab(data['wholesale_tab']),
    );
  }

  int tabFor(String workspace) {
...
// =====================================================
// HPJ AGRICULTURE INTELLIGENCE FEED
// Admin-published, audience-targeted updates for customers,
// farmers and wholesale businesses.
// =====================================================
class AgricultureFeedUpdate {
  final String id;
  final String title;
  ...
  factory AgricultureFeedUpdate.fromSupabase(Map<String, dynamic> data) {
    List<String> parseAudiences(dynamic value) {
      if (value is List) {
        return value
            .map((item) => item.toString().trim().toLowerCase())
            .where((item) => item.isNotEmpty)
    ...
    return AgricultureFeedUpdate(
      id: (data['id'] ?? '').toString(),
      title: (data['title'] ?? '').toString().trim(),
      summary: (data['summary'] ?? '').toString().trim(),
      imageUrl: optionalText(data['image_url']),
      category: (data['category'] ?? 'agriculture_news')
          .toString()
          .trim()
          .toLowerCase(),
      audiences: parseAudiences(data['audiences']),
      priority: (data['priority'] ?? 'normal').toString().trim().toLowerCase(),
      sourceName: optionalText(data['source_name']),
      sourceUrl: optionalText(data['source_url']),
      actionLabel: optionalText(data['action_label']),
      actionType:
          (data['action_type'] ?? 'none').toString().trim().toLowerCase(),
      actionId: optionalText(data['action_id']),
      isActive: data['is_active'] != false,
      publishAt: parseRequiredDate(data['publish_at']),
      expiresAt: parseOptionalDate(data['expires_at']),
      createdBy: optionalText(data['created_by']),
      createdAt: parseOptionalDate(data['created_at']),
      updatedAt: parseOptionalDate(data['updated_at']),
    );
  }

  bool isForAudience(String audience) {
    final clean = audience.trim().toLowerCase();
    return audiences.contains(clean);
  }

  bool get isPublishedNow {
    final now = DateTime.now();
    if (!isActive || publishAt.isAfter(now)) return false;
    return expiresAt == null || expiresAt!.isAfter(now);
  }

  bool get isUrgent => priority == 'urgent';
  bool get isImportant => priority == 'important' || isUrgent;
  bool get hasSource => (sourceUrl ?? '').trim().isNotEmpty;
  bool get hasAction =>
      actionType != 'none' &&
      ((actionLabel ?? '').trim().isNotEmpty || actionType == 'external');
...
// Aggregated national market signal. No farmer or buyer
// identity is exposed to marketplace users.
// =====================================================
class JamaicaSupplyDemandInsight {
  final String cropName;
  final String unit;
  ...
  final List<String> supplyParishes;
  final DateTime? earliestNeedBy;
  final double priorityScore;

  const JamaicaSupplyDemandInsight({
    required this.cropName,
    required this.unit,
    ...
    required this.supplyParishes,
    this.earliestNeedBy,
    required this.priorityScore,
  });

  factory JamaicaSupplyDemandInsight.fromSupabase(
    Map<String, dynamic> data,
  ) {
    ...
    List<String> textList(dynamic value) {
      if (value is List) {
        return value
    ...
    return JamaicaSupplyDemandInsight(
      cropName: (data['crop_name'] ?? '').toString().trim(),
      unit: (data['unit'] ?? 'unit').toString().trim(),
      demandQuantity: number(data['demand_quantity']),
      supplyQuantity: number(data['supply_quantity']),
      gapQuantity: number(data['gap_quantity']),
      coveragePercent: number(data['coverage_percent']),
      farmerCount: whole(data['farmer_count']),
      demandCount: whole(data['demand_count']),
      supplyParishes: textList(data['supply_parishes']),
      earliestNeedBy: date(data['earliest_need_by']),
      priorityScore: number(data['priority_score']),
    );
  }

  bool get hasDemand => demandQuantity > 0.001;
  bool get hasSupply => supplyQuantity > 0.001;
  bool get hasShortage => gapQuantity > 0.001;
  bool get isCriticalShortage =>
      hasDemand && coveragePercent < 50 && gapQuantity > 0.001;
  bool get isTight =>
      hasDemand && coveragePercent >= 50 && coveragePercent < 85;
  bool get isBalanced =>
      hasDemand && coveragePercent >= 85 && coveragePercent <= 120;
  bool get hasSurplus =>
      hasSupply && (!hasDemand || supplyQuantity > demandQuantity * 1.20);

  String get statusLabel {
    if (isCriticalShortage) return 'Critical gap';
    if (isTight) return 'Supply tight';
    if (hasShortage) return 'Supply gap';
  ...
  String get coverageLabel {
    if (!hasDemand && hasSupply) return 'Supply available';
    if (!hasDemand) return 'No planned demand';
    final value = coveragePercent.clamp(0, 999).round();
    return '$value% covered';
  }

  String get formattedDemand => _formatQuantity(demandQuantity);
  String get formattedSupply => _formatQuantity(supplyQuantity);
  String get formattedGap => _formatQuantity(gapQuantity);
